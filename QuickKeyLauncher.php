...
class QuickKeyLauncher extends Parser 
{
    ...
    public function getKeys($number)
    {
        ...
        $my_keys = array();
        if (CoreLocal::get('NoCompat') == 1 || $dbc->tableExists('QuickLookups')) {
            $prep = $dbc->prepare('
                SELECT label,
                    action
                FROM QuickLookups
        ...
        if (count($my_keys) == 0 && file_exists(__DIR__ . '/quickkeys/keys/' . $number . '.php')) {
            include(__DIR__ . '/quickkeys/keys/' . $number . '.php');
        }
